    ...
    <div class="card">
        <form action=" {{ route('question.update', $question->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        ...
                        <textarea class="ckeditor form-control" id="body" name="body"
                            value="{{ $question->body }} "> {{ $question->body }}</textarea>
                        @if ($errors->has('body'))
                ...
                {{-- {{ dd($tags) }} --}}
                @php
                // $dbtag= $question->tag->pluck('id')->toArray()
                $dbtag = explode('"', $question->question_tag);
                @endphp
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        <label for="question_tag">
                            <strong>Select Tags 
                                <span class="text-danger">*</span>
                            </strong>
                        </label>
                        <select class="js-example-basic-multiple o100 form-control" name="question_tag[]" multiple="multiple">
                            <option class="o100" value="" disabled>Select tag</option>
                            @foreach ($tags as $tag)
                            <option class="fc-black-900" value="{{ $tag->tag }}"@if (in_array($tag->tag, $dbtag)) selected @endif>{{ $tag->tag }}</option>
                            @endforeach
                        </select>
